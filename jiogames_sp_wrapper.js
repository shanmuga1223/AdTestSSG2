var adSpotInterstitial = "6oj40yzs";
var adSpotRewardedVideo = "k3tijz1a";
var adSpotInStreamVideo = "";
var package = "com.pixcellplay.superstylistgameSP";
var isAdReady = false;
var isRVReady = false;
...
window.onAdError = function (data, pErrorMessage) {
    ...
    adSpotKey == adSpotInterstitial && (isAdReady = false, console.error("JioGames: onAdError MidRoll " + isAdReady+" errorMessage "+errorMessage));
    adSpotKey == adSpotRewardedVideo && (isRVReady = false, console.error("JioGames: onAdError RewardedVideo " + isRVReady+" errorMessage "+errorMessage));
};
...
window.onAdFailedToLoad = function (data, pDescription){
    ...
    console.error("JioGames: onAdFailedToLoad "+data.toString()+" localData "+localData[0]+" "+localData[1]);
    ...
    if (adSpotKey == adSpotRewardedVideo) {
        ...
        unityGameInstance.SendMessage('Jio', 'OnRewardedAdFailedToShow', data.toString());
    } else {
        console.log("Interstitial Failed to Open js! ");
        unityGameInstance.SendMessage('Jio', 'OnInterstitialAdShownOrClosed', data.toString());
    }
};
